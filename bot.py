...
# ---------------- V3 ADVANCED RISK / REGIME ----------------
def market_regime():
    """Daily NIFTY regime using EMA20/50/200. Returns label and score."""
    try:
        uni=universe_cached()
        inst=next((x for x in uni if x["symbol"] in ("NIFTY 50","NIFTY")),None)
        if not inst: return "UNKNOWN", 0
        rows=candles(inst["key"],400); c=closes(rows)
        if len(c)<200: return "UNKNOWN",0
        e20,e50,e200=ema(c,20),ema(c,50),ema(c,200); ltp=c[-1]
        if ltp>e20>e50>e200: return "BULLISH", 90
        if ltp<e20<e50<e200: return "BEARISH", 20
        return "SIDEWAYS", 50
    except Exception as e:
        log.warning("Regime: %s",e); return "UNKNOWN",0

def position_size(capital, entry, stop, risk_pct=1.0):
    if entry<=0 or stop<=0 or capital<=0: return 0,0
...
def save_signal(x):
    try:
        data=[]
        if HISTORY_FILE.exists(): data=json.loads(HISTORY_FILE.read_text())
        data.append({"time":ist_now().isoformat(),"symbol":x["symbol"],"action":x["action"],"score":x["score"],"entry":x["entry"],"stop":x.get("stop"),"t1":x.get("t1"),"t2":x.get("t2")})
        HISTORY_FILE.write_text(json.dumps(data[-2000:],indent=2))
    except Exception as e: log.warning("History: %s",e)

def news_severity(ns):
    if ns>=5: return "MAJOR POSITIVE"
    if ns>=2: return "POSITIVE"
    if ns<=-5: return "MAJOR NEGATIVE"
    if ns<=-2: return "NEGATIVE"
    return "NEUTRAL"

def backtest_symbol(symbol, days=250):
    inst=find_inst(symbol)
    if not inst: return None
    rows=candles(inst["key"], min(400,max(60,days)))
    c=closes(rows)
    if len(c)<55: return None
    # Simple, transparent EMA20/EMA50 crossover backtest; informational only.
    cash=100000.0; shares=0; entry=0; trades=[]
    for i in range(50,len(c)):
        e20=ema(c[:i+1],20); e50=ema(c[:i+1],50); price=c[i]
        if shares==0 and e20>e50:
            shares=int(cash/price); entry=price; cash-=shares*price
        elif shares and e20<e50:
            cash+=shares*price; trades.append((entry,price)); shares=0
    if shares: cash+=shares*c[-1]; trades.append((entry,c[-1]))
    wins=sum(1 for a,b in trades if b>a); total=len(trades); ret=(cash/100000-1)*100
    return total,wins,ret
...
def run_scan():
    ...
    lines=[f"📊 NSE STOCK SCAN | {ist_now().strftime('%d-%m-%Y %H:%M IST')}",
           ...
           ""]
    lines.append(f"Market regime: {regime} ({regime_score}/100)")
    if buys:
...
# ---------------- TELEGRAM COMMANDS ----------------
COMMAND_HELP = """🤖 UPSTOX NSE STOCK BOT V3
...
/pnl - Portfolio P&L
/risk CAPITAL - Position sizing at 1% risk
/regime - NIFTY market regime
/backtest SYMBOL - Simple EMA backtest
...
DAILY_RISK_LIMIT=float(os.getenv("DAILY_RISK_LIMIT", "2"))
HISTORY_FILE=Path(os.getenv("SIGNAL_HISTORY_FILE", "signal_history.json"))
SEEN_ALERTS=set()
...
def handle_command(chat_id,text):
    ...
    try:
        if cmd in ("/start","/help"): msg=COMMAND_HELP
            ...
            rg,rs=market_regime(); msg=f"📈 MARKET REGIME\n{rg} | Score {rs}/100"
        elif cmd=="/risk":
            if not arg: msg="Use: /risk 10000 (uses latest watchlist top setup)"
            else:
                capital=float(arg); xs=command_scan_subset("top"); x=xs[0] if xs else None
                if not x or not x.get("stop"): msg="No valid setup available."
                else:
                    q,risk=position_size(capital,x["entry"],x["stop"],1.0); msg=f"💰 RISK PLAN\n{x['symbol']}\nCapital ₹{capital:,.0f}\nRisk 1%: ₹{capital*0.01:,.0f}\nQty: {q}\nApprox risk: ₹{risk:,.0f}\nEntry ₹{x['entry']:.2f} | SL ₹{x['stop']:.2f}"
        elif cmd=="/backtest":
            if not arg: msg="Use: /backtest RELIANCE"
            else:
                bt=backtest_symbol(arg); msg=(f"🧪 BACKTEST {arg}\nTrades: {bt[0]}\nWins: {bt[1]}\nWin rate: {(bt[1]/bt[0]*100 if bt[0] else 0):.1f}%\nReturn on ₹1L: {bt[2]:+.2f}%\n\nEMA20/EMA50 crossover only; historical, not a forecast." if bt else "Backtest unavailable.")
        elif cmd=="/history":
            try:
                d=json.loads(HISTORY_FILE.read_text()) if HISTORY_FILE.exists() else []
                msg="📜 SIGNAL HISTORY\n"+"\n".join(f"{x['symbol']} | {x['action']} | {x['score']} | {x['time'][:16]}" for x in d[-15:]) if d else "No signal history yet."
            except Exception: msg="History unavailable."
        elif cmd=="/status": msg=f"🤖 Bot V3 OK\nUniverse: {len(universe_cached())} NSE equities\nScheduled alerts: {'ON' if ALERTS_ENABLED else 'OFF'}\nF&O: DISABLED\nOrders: NEVER\nDaily risk limit: {DAILY_RISK_LIMIT}%"
